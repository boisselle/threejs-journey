...
void main()
{
    ...
    // barY *= step(0.5, mod(vUv.y * 10.0, 1.0));
    ...
    // Pattern 22
    // float strength = floor(vUv.x * 9.0) / 10.0;
    // strength *= floor(vUv.y * 9.0) / 10.0;
    // gl_FragColor = vec4(strength, strength, strength, 1.0);
    ...
    // Pattern 43
    // Would be sweet to have this with angle *= 100.0 but move when you move the mouse
    // float angle = atan(vUv.x - 0.5, vUv.y - 0.5);
    // angle /= PI * 2.0;
    // angle += 0.5;
    // angle *= 5.0;
    ...
    // Pattern 49
    // float strength = sin(cnoise(vUv * 10.0) * 14.0);

    // gl_FragColor = vec4(strength, strength, strength, 1.0);

    // Pattern 49
    // float strength = step(0.9, sin(cnoise(vUv * 10.0) * 14.0));
